(function($) {
	...
	var MagicSuggest = function(element, options) {
			...
			var defaults = {
				...
				emptyTextCls: "ms-empty-text form-control",
				expanded: false,
			...
			var self = {
				...
				_render: function(el) {
					...
					ms.container = $("<div/>", {
						id: cfg.id,
						"class": "ms-ctn " + cfg.cls + (cfg.disabled === true ? " ms-ctn-disabled" : "") + (cfg.editable === true ? "" : " ms-ctn-readonly"),
						style: cfg.style
					}).width(w);
					...
					$("body").click(function(e) {
						if (ms.container.hasClass("ms-ctn-bootstrap-focus") && ms.container.has(e.target).length === 0 && e.target.className.indexOf("ms-res-item") < 0 && e.target.className.indexOf("ms-close-btn") < 0 && ms.container[0] !== e.target) {
							handlers._onBlur()
						}
					});
					if (cfg.expanded === true) {
